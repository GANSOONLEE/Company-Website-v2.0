...
<head>
    ...
    <link rel="preload" href="//cdnjs.cloudflare.com/ajax/libs/x-editable/1.5.0/bootstrap3-editable/css/bootstrap-editable.css" rel="stylesheet"/>
    ...
    <div class="container-fluid">
        <div class="navbar-header">
            ...
            <a href="<?php echo action('\Barryvdh\TranslationManager\Controller@getIndex') ?>" class="navbar-brand">
                {{ __('translation.translation-manager') }}
            </a>
        </div>
    </div>
</header>
<div class="container-fluid">
    ...
    <p>
                ...
                <div class="row">
                    <div class="col-sm-3">
                        <select name="replace" class="form-control">
                            <option value="0">{{ __('translation.append-new-translation') }}</option>
                            <option value="1">{{ __('translation.replace-existing-translation') }}</option>
                        </select>
                    </div>
                    <div class="col-sm-2">
                    <button type="submit" class="btn btn-success btn-block"  data-disable-with="Loading..">{{ __('translation.import-groups') }}</button>
                    </div>
                </div>
            </div>
        </form>
        <form class="form-find" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postFind') ?>" data-remote="true" role="form" data-confirm="Are you sure you want to scan you app folder? All found translation keys will be added to the database.">
            <div class="form-group">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                <button type="submit" class="btn btn-info" data-disable-with="Searching.." >{{ __('translation.find-translation-in-files') }}</button>
            </div>
        </form>
        <?php endif; ?>
        <?php if(isset($group)) : ?>
            <form class="form-inline form-publish" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postPublish', $group) ?>" data-remote="true" role="form" data-confirm="Are you sure you want to publish the translations group '<?php echo $group ?>? This will overwrite existing language files.">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                <button type="submit" class="btn btn-info" data-disable-with="Publishing.." >{{ __('translation.publish-translations') }}</button>
                <a href="<?= action('\Barryvdh\TranslationManager\Controller@getIndex') ?>" class="btn btn-default">{{ __('translation.back') }}</a>
            </form>
        <?php endif; ?>
    </p>
    <form role="form" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postAddGroup') ?>">
        <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
        <div class="form-group">
            <p>{{ __('translation.choose-group') }}</p>
        ...
        <div class="form-group">
            <label>{{ __('translation.new-group') }}</label>
            <input type="text" class="form-control" name="new-group" />
        </div>
        <div class="form-group">
            <input type="submit" class="btn btn-default" name="add-group" value="{{ __('translation.add-and-edit-key') }}" />
        </div>
    </form>
    <?php if($group): ?>
        <form action="<?php echo action('\Barryvdh\TranslationManager\Controller@postAdd', array($group)) ?>" method="POST"  role="form">
            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            <div class="form-group">
                <label>{{ __('translation.add-new-keys-to-this-group') }}</label>
                <textarea class="form-control" rows="3" name="keys" placeholder="{{ __('translation.placeholder') }}"></textarea>
            </div>
            <div class="form-group">
                <input type="submit" value="{{ __('translation.add-keys') }}" class="btn btn-primary">
            </div>
        </form>
        <div class="row">
            <div class="col-sm-2">
                <span class="btn btn-default enable-auto-translate-group">{{ __('translation.use-auto-translate') }}</span>
            </div>
        </div>
        <form class="form-add-locale autotranslate-block-group hidden" method="POST" role="form" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postTranslateMissing') ?>">
            ...
            <div class="row">
                <div class="col-sm-6">
                    <div class="form-group">
                        <label for="base-locale">{{ __('translation.base-locale-for-auto-translations') }}</label>
                    ...
                    <div class="form-group">
                        <label for="new-locale">{{ __('translation.enter-targer-locale-key') }}</label>
                        <input type="text" name="new-locale" class="form-control" id="new-locale" placeholder="{{ __('translation.enter-targer-locale-key') }}" />
                    </div>
                    <?php if(!config('laravel_google_translate.google_translate_api_key')): ?>
                        <p>
                            <code>{{ __('translation.google-translate-api') }}</code>
                        </p>
                    <?php endif; ?>
                    <div class="form-group">
                        <input type="hidden" name="with-translations" value="1">
                        <input type="hidden" name="file" value="<?= $group ?>">
                        <button type="submit" class="btn btn-default btn-block"  data-disable-with="{{ __('translation.adding') }}">{{ __('translation.auto-translate-missing-key') }}</button>
                    </div>
                </div>
            </div>
        </form>
        <hr>
    <h4>{{ __('translation.total') }} <?= $numTranslations ?>, {{ __('translation.change') }} <?= $numChanged ?></h4>
        <table class="table">
            <thead>
            <tr>
                <th width="15%">{{ __('translation.key') }}</th>
                <?php foreach ($locales as $locale): ?>
                    <th><?= $locale ?></th>
                <?php endforeach; ?>
                    ...
                    <?php foreach ($locales as $locale): ?>
                        ...
                        <td>
                            <a href="#edit"
                               ...
                               data-title="{{ __('translation.enter-translation') }}"><?php echo $t ? htmlentities($t->value, ENT_QUOTES, 'UTF-8', false) : '' ?></a>
                        </td>
                    <?php endforeach; ?>
                    <?php if ($deleteEnabled): ?>
                        <td>
    ...
    <?php else: ?>
        <fieldset>
            <legend>{{ __('translation.support-locales') }}</legend>
            <p>
                {{ __('translation.current-support-locales') }}
            </p>
            ...
            <form class="form-add-locale" method="POST" role="form" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postAddLocale') ?>">
                ...
                <div class="form-group">
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-2">
                            <button type="submit" class="btn btn-default btn-block"  data-disable-with="Adding..">{{ __('translation.add-locales') }}</button>
                        </div>
                    </div>
                </div>
            </form>
        ...
        <fieldset>
            <legend>{{ __('translation.export-all-translation') }}</legend>
            <form class="form-inline form-publish-all" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postPublish', '*') ?>" data-remote="true" role="form" data-confirm="Are you sure you want to publish all translations group? This will overwrite existing language files.">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                <button type="submit" class="btn btn-primary" data-disable-with="Publishing.." >{{ __('translation.publish-all') }}</button>
            </form>
        </fieldset>
